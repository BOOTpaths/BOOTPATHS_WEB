...
export default function AdminConsole({ 
  ...
  const DEMO_EMAILS = [
    'anand.varma@gmail.com',
    'meera.nair@outlook.com',
    'rohan.sharma@techcorp.in',
    'deepa.krishnan@yahoo.com',
    'deepa.k@yahoo.com'
  ];

  const isDemoBooking = (booking) => {
    if (!booking) return false;
    if (booking.isMock === true) return true;
    const pId = (booking.paymentId || '').toLowerCase();
    if (pId.includes('pay_demo') || pId.includes('test') || pId.startsWith('pay_p849') || pId.startsWith('pay_m392') || pId.startsWith('pay_r774') || pId.startsWith('pay_d519')) {
      return true;
    }
    const email = (booking.userEmail || booking.email || '').toLowerCase().trim();
    if (DEMO_EMAILS.some(demoEmail => email === demoEmail || email.includes(demoEmail))) {
      return true;
    }
    return false;
  };

  // Only display genuine bookings saved from real form submissions and live/test Razorpay checkouts
  const displayBookings = adminBookings.filter(b => !isDemoBooking(b));

  const handlePurgeDemoBookings = async () => {
    if (!window.confirm('Purge all demo and mock bookings from Firestore? This will permanently delete test reservation records.')) return;
    
    setIsPurgingDemo(true);
    let purgedCount = 0;
    try {
      const demoList = adminBookings.filter(isDemoBooking);
      for (const item of demoList) {
        if (item.id) {
          try {
            ...
            console.warn('Failed deleting demo booking document:', item.id, err);
          }
        }
      }
      setAdminBookings(prev => prev.filter(b => !isDemoBooking(b)));
      alert(`Purged ${purgedCount} demo booking records from the database.`);
    } catch (err) {
      console.error('Purge demo bookings error:', err);
        ...
        <div className="bg-[#FFFFFF] border border-[#E7E7E4] rounded-2xl p-6 shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              ...
              className="bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 px-4 py-2.5 rounded-xl font-bold text-xs uppercase tracking-wider shadow-sm transition-all flex items-center gap-2 cursor-pointer font-outfit disabled:opacity-50"
              title="Permanently remove mock / demo bookings from database"
            >
              <Trash2 className="h-4 w-4 text-rose-600" />
              <span>{isPurgingDemo ? 'Purging...' : '🗑️ Purge Demo Bookings'}</span>
            </button>
            <button
