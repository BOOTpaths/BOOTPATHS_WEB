...
// -------------------------------------------------------------
// 6. SINGLE PERSON EXPEDITION TARIFF & WHATSAPP DISPATCH
// -------------------------------------------------------------
const PRICE_PER_PERSON = 1950;

function initSinglePersonBooking() {
  const dynamicBtn = document.getElementById('dynamicBookingBtn');
  if (dynamicBtn) {
    const msg = encodeURIComponent('Hi Bootpaths, I would like to reserve the Kolukkumala Sunrise & 4x4 Jeep Expedition. Tariff: ₹' + PRICE_PER_PERSON.toLocaleString('en-IN') + ' per person (All-Inclusive 4x4 Jeep + Forest Permits + Factory Tour + Breakfast). Please share slot availability and reservation confirmation.');
